...
router.get("/", requireAuth, checkPermission("demands", "view"), async (req, res) => {
    const { status, priority, epic, responsible, externalOwnerId, overdue, nextFrom, nextTo } = req.query;
    ...
    if (nextFrom || nextTo) {
        ...
        if (nextFrom)
            filter.nextFollowUpAt = { ...filter.nextFollowUpAt, $gte: new Date(nextFrom) };
        if (nextTo)
            filter.nextFollowUpAt = { ...filter.nextFollowUpAt, $lte: new Date(nextTo) };
    }
    if (overdue === "true") {
...
router.post("/", requireAuth, checkPermission("demands", "create"), async (req, res) => {
    ...
    const now = new Date();
    const sponsor = typeof req.body.sponsor === "string" && req.body.sponsor.trim()
...
router.patch("/:id", requireAuth, checkPermission("demands", "edit"), async (req, res) => {
    ...
    if (payload.nextFollowUpAt)
        payload.nextFollowUpAt = new Date(payload.nextFollowUpAt);
    if (payload.lastContactAt)
        payload.lastContactAt = new Date(payload.lastContactAt);
    if (payload.lastUpdate)
        payload.lastUpdate = new Date(payload.lastUpdate);
    if (Array.isArray(payload.tasks)) {
        const total = payload.tasks.length;
        const completed = payload.tasks.filter((task) => task?.isCompleted).length;
    ...
    const demand = await DemandModel.findByIdAndUpdate(req.params.id, { ...payload, audits }, { new: true });
    return res.json(demand);
});
router.post("/:id/contact", requireAuth, checkPermission("demands", "edit"), async (req, res) => {
    const actor = res.locals.user?.email ?? "system";
    const { channel, summary, nextFollowUpAt } = req.body;
...
router.post("/:id/comment", requireAuth, checkPermission("demands", "edit"), async (req, res) => {
    ...
    const demand = await DemandModel.findByIdAndUpdate(req.params.id, {
