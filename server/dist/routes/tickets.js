...
const QUEUES = ["TI Interna", "Fornecedor"];
const STATUSES = [
    "Novo",
    "Triagem",
    "Em atendimento",
...
const IMPACT = ["Baixo", "Médio", "Alto"];
const URGENCY = ["Baixa", "Média", "Alta"];
...
function calcPriority(impact, urgency) {
    ...
    if (highImpact && highUrg)
        return "P0";
    if ((highImpact && medUrg) || (medImpact && highUrg))
        return "P1";
    if ((highImpact && !highUrg && !medUrg) || (medImpact && medUrg) || (!highImpact && !medImpact && highUrg))
        return "P2";
    return "P3";
}
function impactUrgencyFromPriority(priority) {
    if (priority === "P0")
        return { impact: "Alto", urgency: "Alta" };
    if (priority === "P1")
        return { impact: "Alto", urgency: "Média" };
    if (priority === "P2")
        return { impact: "Médio", urgency: "Média" };
    return { impact: "Baixo", urgency: "Baixa" };
}
function calcSlaDueAt(priority, openedAt, policies) {
    ...
    if (priority === "P0")
        return new Date(now.getTime() + urgent * 60 * 60 * 1000);
    if (priority === "P1")
        return new Date(now.getTime() + high * 60 * 60 * 1000);
    if (priority === "P2")
        return new Date(now.getTime() + medium * 60 * 60 * 1000);
...
function canApproveTicket(args) {
    if (!args.userId)
        return false;
    if (args.approverId)
        return args.approverId === args.userId;
...
function isPortalUser(profile) {
    if (!profile?.permissions)
        return false;
    const perms = profile.permissions;
    return (perms.tickets?.view &&
        perms.tickets?.create &&
        !perms.tickets?.edit &&
        !perms.tickets?.delete &&
        !perms.settings?.manage);
}
function canAccessTicket(ticket, userId) {
    if (!ticket)
        return false;
    if (!userId)
        return false;
...
async function notifyApprovers({ service, ticket, requester, }) {
...
async function nextTicketCode() {
    ...
    const counter = await CounterModel.findOneAndUpdate({ name: `ticket-${year}` }, { $inc: { seq: 1 } }, { new: true, upsert: true });
    const seq = String(counter.seq).padStart(6, "0");
    return `CH-${year}-${seq}`;
}
router.get("/", requireAuth, checkPermission("tickets", "view"), async (req, res) => {
    const { system, status, demandId, relatedAssetId } = req.query;
    const { queue, overdue, unlinked } = req.query;
    const filter = {};
    ...
    if (relatedAssetId)
        filter.relatedAssetId = relatedAssetId;
    if (queue)
        filter.queue = queue;
    if (unlinked === "true")
        filter.demandId = null;
    let tickets = await TicketModel.find(filter).sort({ updatedAt: -1 });
    if (overdue === "true") {
        tickets = tickets.filter((ticket) => ticket.isSlaOverdue);
    }
...
router.post("/", requireAuth, checkPermission("tickets", "create"), async (req, res) => {
    ...
    const slaDueAt = status === "Aguardando aprovação"
        ? null
...
router.patch("/:id", requireAuth, checkPermission("tickets", "edit"), async (req, res) => {
    ...
    const nextPriority = PRIORITIES.includes(payload.priority)
        ? String(payload.priority)
        : calcPriority(nextImpact, nextUrgency);
    if (!STATUSES.includes(nextStatus)) {
        return res.status(400).json({ message: "Status inválido." });
    }
    if (!QUEUES.includes(nextQueue)) {
        return res.status(400).json({ message: "Fila inválida." });
    }
...
router.post("/:id/comment", requireAuth, checkPermission("tickets", "edit"), async (req, res) => {
    ...
    const updated = await TicketModel.findByIdAndUpdate(req.params.id, { $push: { comments: { at: new Date(), author, message } } }, { new: true });
    return res.json(updated);
});
router.post("/:id/link-demand", requireAuth, checkPermission("tickets", "edit"), async (req, res) => {
    ...
    const updated = await TicketModel.findByIdAndUpdate(req.params.id, { demandId }, { new: true });
    return res.json(updated);
});
...
router.get("/:id/emails", requireAuth, checkPermission("tickets", "view"), async (req, res) => {
    ...
    const items = [
        ...inbound.map((item) => ({
            ...
            attachments: item.attachments?.map((att, index) => ({
                index,
...
router.get("/:id/emails/:emailId/attachment/:index", requireAuth, checkPermission("tickets", "view"), async (req, res) => {
    if (isPortalUser(res.locals.user?.profile)) {
        return res.status(403).json({ message: "Ação restrita para solicitantes." });
    }
...
router.post("/:id/email", requireAuth, checkPermission("tickets", "edit"), upload.array("files"), async (req, res) => {
    if (isPortalUser(res.locals.user?.profile)) {
        return res.status(403).json({ message: "Ação restrita para solicitantes." });
    }
    const { to, subject, body, cc, bcc, statusAfter } = req.body;
    ...
    try {
        ...
        if (statusAfter && STATUSES.includes(statusAfter)) {
