...
DemandSchema.virtual("isOverdue").get(function () {
    if (this.status !== "Aguardando terceiros")
        return false;
    if (!this.nextFollowUpAt)
        return false;
    return new Date(this.nextFollowUpAt).getTime() <= Date.now();
});
DemandSchema.virtual("agingDays").get(function () {
    if (!this.createdAt)
        return 0;
    const diff = Date.now() - new Date(this.createdAt).getTime();
    return Math.floor(diff / (1000 * 60 * 60 * 24));
});
DemandSchema.virtual("tasksProgress").get(function () {
