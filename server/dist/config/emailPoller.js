...
function buildThreadKey(subject, from) {
    return `${normalizeSubject(subject)}|${from.toLowerCase()}`;
}
function hashMessageFallback(subject, from, date) {
...
async function pollOnce() {
    ...
    try {
        ...
        for await (const msg of client.fetch(uids, { envelope: true, source: true })) {
            ...
            const impact = IMPACT.includes(config.defaultImpact) ? config.defaultImpact : "Médio";
            const urgency = URGENCY.includes(config.defaultUrgency) ? config.defaultUrgency : "Média";
            let queue = QUEUES.includes(config.defaultQueue) ? config.defaultQueue : "TI Interna";
