...
async function ensureAdminUser() {
    ...
    if (!existing.profile)
        updates.profile = adminProfile.id;
    if (typeof existing.isActive !== "boolean")
        updates.isActive = true;
    if (typeof existing.mustChangePassword !== "boolean")
...
async function migrateUsersToProfiles() {
    ...
    if (!adminProfile || !techProfile || !requesterProfile)
        return;
    ...
    for (const user of users) {
        ...
        if (oldRole === "Agent")
            targetProfileId = techProfile.id;
