...
router.get("/", requireAuth, checkPermission("settings", "manage"), async (_req, res) => {
  ...
  return res.json({
    ...
    portalWelcomeTitle: params.portalWelcomeTitle ?? "Olá!",
    portalWelcomeSubtitle: params.portalWelcomeSubtitle ?? "Como o time de TI pode facilitar o seu dia hoje? Escolha um serviço ou busque ajuda rápida.",
    portalLogoUrl: params.portalLogoUrl ?? "",
...
router.put("/", requireAuth, checkPermission("settings", "manage"), async (req, res) => {
  try {
    ...
    const update: Record<string, unknown> = {
      ...
      portalWelcomeTitle: payload.portalWelcomeTitle ?? params.portalWelcomeTitle ?? "Olá!",
      portalWelcomeSubtitle: payload.portalWelcomeSubtitle ?? params.portalWelcomeSubtitle ?? "Como o time de TI pode facilitar o seu dia hoje? Escolha um serviço ou busque ajuda rápida.",
      portalLogoUrl: payload.portalLogoUrl ?? params.portalLogoUrl ?? "",
    ...
    return res.json({
      ...
      portalWelcomeTitle: updated?.portalWelcomeTitle ?? "Olá!",
      portalWelcomeSubtitle: updated?.portalWelcomeSubtitle ?? "Como o time de TI pode facilitar o seu dia hoje? Escolha um serviço ou busque ajuda rápida.",
      portalLogoUrl: updated?.portalLogoUrl ?? "",
      cannedResponses: updated?.cannedResponses ?? [],
    });
  } catch (error: any) {
    console.error("Erro ao atualizar parâmetros do sistema:", error);
    return res.status(500).json({ message: error?.message || "Erro interno ao salvar parâmetros." });
  }
});
