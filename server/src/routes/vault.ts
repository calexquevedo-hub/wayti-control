...
function buildAudit(req: any, recordId: string, action: string) {
  const user = req.res?.locals?.user as { email: string } | undefined;
  return {
    recordId,
    action,
...
router.get("/", requireAuth, checkPermission("settings", "manage"), async (req, res) => {
  const { search, tag, externalOwnerId } = req.query as Record<string, string | undefined>;
...
router.post("/", requireAuth, checkPermission("settings", "manage"), async (req, res) => {
  ...
  let notesEnc: { iv: string; data: string; tag: string } | null = null;
  try {
