...
router.get(
  "/check-renewals/summary",
  requireAuth,
  checkPermission("contracts", "view"),
  async (_req, res) => {
    const contracts = await ContractModel.find().lean({ virtuals: true });
    const critical = contracts.filter((c: any) => c.renewalStatus === "Critical").length;
    const monthlyTotal = contracts
      .filter((c: any) => c.status === "Active" && c.frequency === "Monthly")
      .reduce((sum: number, c: any) => sum + (c.costValue ?? 0), 0);
